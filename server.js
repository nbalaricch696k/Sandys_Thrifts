  ...
  return new Promise((resolve, reject) => {
    const uploadStream = cloudinary.uploader.upload_stream({
      ...
      eager: [
        ...
        { width: 480, height: 853, crop: 'fill', format: 'mp4' }
      ]
    }, (error, result) => {
...
async function triggerCacheClearHook() {
  const hookUrl = process.env.CACHE_CLEAR_HOOK_URL || process.env.VERCEL_DEPLOY_HOOK || process.env.RENDER_DEPLOY_HOOK;
  if (!hookUrl) return;
  try {
    await axios.post(hookUrl, {});
  } catch (error) {
    console.warn('Cache clear hook failed:', error.message || error);
  }
}
...
async function provisionInitialAdmin(adminEmail, defaultPassword = 'admin123') {
  ...
  console.log(`Initial admin provisioned: Username: ${username}, Email: ${adminEmail}`);
...
app.get('/api/products', (req, res) => {
  ...
  if (category) {
    products = products.filter(p => {
      ...
      if (category === 'essentials' || category === 'best-sellers') return true;
...
// Admin setup info endpoint
app.get('/api/admin/setup-info', (req, res) => {
  const settings = readJSON(SETTINGS_FILE) || {};
  const isFirstSetup = !settings.adminPasswordHash || !settings.adminUsername;
  res.json({
    isFirstSetup,
...
app.get('/api/admin/categories', requireAdmin, (req, res) => {
  const settings = readJSON(SETTINGS_FILE) || {};
  res.json(settings.categories || ['Essentials', 'Jerseys', 'T-Shirts', 'Tracksuits']);
});
...
app.post('/api/admin/categories', requireAdmin, (req, res) => {
  ...
  const settings = readJSON(SETTINGS_FILE) || {};
  settings.categories = settings.categories || ['Essentials', 'Jerseys', 'T-Shirts', 'Tracksuits'];
  const normalized = category.trim();
  const exists = settings.categories.some(cat => cat.toLowerCase() === normalized.toLowerCase());
  if (exists) {
    return res.status(409).json({ error: 'Category already exists' });
  }
  settings.categories.push(normalized);
  writeJSON(SETTINGS_FILE, settings);

  res.json({ ok: true, categories: settings.categories });
});
...
app.post('/admin/login', async (req, res) => {
  ...
  const isAdminIdentifier = identifier === adminUsername.toLowerCase() || identifier === adminEmail.toLowerCase();
  
  if (!adminPasswordHash) {
    // Fallback to plain text if not hashed yet
    if (isAdminIdentifier && password === (settings.adminPassword || 'admin')) {
    ...
    // Use hashed password
    if (isAdminIdentifier && await bcrypt.compare(password, adminPasswordHash)) {
      req.session.adminLoggedIn = true;
      res.redirect('/admin/dashboard');
    } else {
      res.status(401).send('Invalid credentials');
...
app.post('/api/admin/products', requireAdmin, (req, res) => {
  ...
  newProduct.category = newProduct.category || 'Essentials';
...
app.put('/api/admin/products/:id', requireAdmin, (req, res) => {
  ...
  updated.discount = parseInt(updated.discount) || 0;
  updated.category = updated.category || products[index].category || 'Essentials';
...
app.get('/api/admin/settings', requireAdmin, (req, res) => {
  ...
  const { adminPassword, adminPasswordHash, ...safeSettings } = settings;
  safeSettings.categories = safeSettings.categories || ['Essentials', 'Jerseys', 'T-Shirts', 'Tracksuits'];
...
app.post('/api/admin/update-hero-video', requireAdmin, upload.single('heroVideo'), async (req, res) => {
  if (!req.file) {
  ...
  if (!allowedTypes.includes(req.file.mimetype)) {
    return res.status(400).json({ error: 'Unsupported format. Use MP4 or WebM.' });
  }

  try {
    const uploadResult = await uploadVideoToCloudinary(req.file.buffer, req.file.originalname);
    ...
    await triggerCacheClearHook();

    res.json({ ok: true, heroVideoUrl: videoUrl });
  } catch (error) {
    console.error('Hero video upload failed:', error);
    res.status(500).json({ error: 'Failed to upload hero video', details: error.message });
  }
});

app.post('/api/admin/settings/change-credentials', requireAdmin, async (req, res) => {
  ...
  if (adminPasswordHash) {
    isCurrentValid = await bcrypt.compare(currentPassword, adminPasswordHash);
  } else {
    // Fallback for plain text
    isCurrentValid = currentPassword === (settings.adminPassword || 'admin');
  }
...
app.post('/api/admin/accounts/change-password', requireAdmin, async (req, res) => {
  ...
  res.json({ ok: true, message: 'Password updated successfully. Please log in again.' });
});
...
app.get('/api/store-info', (req, res) => {
  ...
  res.json({
    ...
    categories: settings.categories || ['Essentials', 'Jerseys', 'T-Shirts', 'Tracksuits']
  });
});
...
app.get('/auth/google/callback', (req, res) => {
  ...
  // Store code in session for profile setup
  req.session.googleCode = code;
  res.redirect('/auth/setup-profile');
});
...
app.post('/auth/create-profile', async (req, res) => {
  ...
  // For MVP: Create a basic user with the username
  // In production, this would exchange the Google code for a token and get user info
  const users = readJSON(USERS_FILE) || [];
  ...
  const user = {
    id: Date.now(),
    name: username,
    email: `user_${Date.now()}@oauth.local`,
    password: 'oauth_user',
    orders: [],
    createdAt: new Date().toISOString()
  };
...
app.post('/webhooks/paystack', async (req, res) => {
  try {
    ...
    if (event.event === 'charge.success') {
      ...
      await sendOrderConfirmationEmail(order, estimatedDeliveryDate);
      order.emailSent = true;
      order.emailSentAt = new Date().toISOString();
...
// SPA fallback for client-side routes like /category/tracksuits
app.get('*', (req, res) => {
  res.sendFile(path.join(__dirname, 'public', 'index.html'));
});
